"""
Generate the Accessibility Conformance Report (ACR) as a .docx file.

Source of truth: docs/accessibility-conformance.md.
Output: docs/accessibility-conformance.docx — the customer-facing /
compliance-archive copy.
...
SOURCE_PATH = REPO_ROOT / "docs" / "accessibility-conformance.md"
OUTPUT_PATH = REPO_ROOT / "docs" / "accessibility-conformance.docx"
...
def assert_structure(blocks: list[tuple[str, object]]) -> None:
    ...
    assert h1s[0].startswith("Accessibility Conformance Report"), (
        f"H1 should be the ACR title, got: {h1s[0]!r}"
    )
    ...
    assert len(tables) >= 6, (
        f"Expected at least 6 tables (conformance levels, evaluation methods, "
        f"4 WCAG principle tables, known limitations, revision history). "
        f"Got {len(tables)}."
    )
