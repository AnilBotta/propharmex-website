# AGENTS.md — Propharmex Website

Project-level instructions for every Codex session working in this repo. **Read top to bottom before touching any file.** These rules override defaults.

---

## 1. Identity & mission

**Propharmex** is a **Canadian pharmaceutical services company**. Our operations are anchored at our Mississauga, Ontario site under our Health Canada Drug Establishment Licence, with 3PL distribution running from the same facility. Our Canadian programmes are supported by an Indian development centre in Hyderabad, Telangana — operational depth in formulation, method development, analytical services, and stability work. The regulatory anchor and the client-of-record relationship sit in Canada. Capabilities span end-to-end drug development, analytical services, regulatory affairs, and 3PL distribution. Our clients are drug developers globally — from US generic sponsors to multilateral procurement agencies. Hyderabad is named only on `/facilities`, `/facilities/hyderabad-india`, and `/quality` (where the QMS scope honestly requires it). Everywhere else, the lead identity is Canadian. Never frame Propharmex as "two-hub" or as a Canada–India intermediary.
...
| AI | Vercel AI SDK + Anthropic Codex (primary) + OpenAI `text-embedding-3-large` (embeddings) + OpenAI GPT-4o (fallback) |
...
Phase 0 (foundation) creates `.Codex/`, `docs/`, AGENTS.md, `.env.example`, `.gitignore`, `.nvmrc`. Phase 2 (Prompt 1) creates `apps/`, `packages/`, `package.json`, `pnpm-workspace.yaml`. **Do not create Next.js or Sanity code before Prompt 1.**
...
- **Vercel MCP** — use for deploys (`deploy_to_vercel`), log pulls (`get_runtime_logs`, `get_deployment_build_logs`), and doc search. Prefer MCP over the `vercel` CLI for Codex-initiated actions.
...
- **Codex-in-Chrome / computer-use** — live UX review and accessibility spot-checks on a running preview.
...
| Phase | Prompts | Gate |
|---|---|---|
| 0. Foundation | this setup | AGENTS.md + 12 skills + docs + `.env.example` exist |
